...
| **Scatterer** *(검증됨)* | 대기 산란 + 자체 카메라 레벨 TAA | `ADAPTER` — 플레어/대기는 코드 없이도 우리 천체 수차 렌더 창을 따라옵니다(카메라별 읽기, 소스 검증). 문제는 **내장 TAA**입니다(자기 설정에서 기본 켜짐, near/far/scaled 카메라에 붙는 일반 MonoBehaviour라 PostProcessLayer 스캔에 안 잡힘) — 매 프레임 투영행렬을 지터시키는데, 비주얼의 빔 증폭이 이를 하늘 자글거림으로 드러냅니다. `ScattererTAAAdapter`가 비주얼이 켜진 동안만 해당 컴포넌트를 꺼두고 끝나면(히스토리 리셋과 함께) 복원합니다. `Scatterer.TemporalAntiAliasing` @ c2d0b0f 기준 소프트 타입, Scatterer 없으면 휴면. `dopplerSuppressScattererTAA=false`로 끌 수 있고, Scatterer의 SMAA는 영향 없습니다. 여기에 더해 **대체 선플레어**(근거리 카메라가 그리는 소프트 가산 스크린 메시)는 매 프레임 단독 캡처해서(`ScattererFlareCapture`, `SunFlare.sunflareGameObject` @ c2d0b0f 기준 소프트 타입) 원본에서 분리 → 깨끗한 하늘만 처리 → 도플러 편이된 플레어로 재합성합니다 — 픽셀 단위 정밀 처리이고, 리플렉션이 어긋나면 콘 윈도 폴백으로 내려앉습니다. |
