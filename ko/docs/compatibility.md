...
| **Kopernicus** (행성 팩들의 엔진) | 항성/행성 시스템 로더 | `ADAPTER` - 게임플레이 쪽은 여전히 무관하지만(행성 데이터를 읽지 않음), 비주얼의 천체 수차가 Kopernicus의 별당 플레어를 함께 재조준합니다. KopernicusSunFlare는 *방향*으로 조준되고 카메라마다 onPreCull에서 다시 덮어쓰므로, 우리 렌더 창 안에서 수차 방향을 재확정합니다(소프트 타입 리플렉션, Release-247 기준 검증, Kopernicus 없으면 휴면). 선플레어 실드는 **가장 가까운 별**을 조준합니다 - 다중성계 팩에서 `Planetarium.fetch.Sun`은 별 아닌 바리센터 루트일 수 있어서입니다. 다중성계 팩 설치 상태의 실행은 아직입니다. |
| **Singularity** *(검증됨 @ 3034093)* | 블랙홀 렌징 (blackrack) | `COMPATIBLE`/`CONSIDER` - 구조적으로 깨끗하게 합성됩니다. 렌즈가 천체 scaledBody에 붙은 실제 스케일드 스페이스 지오메트리라 우리 천체 워프가 블랙홀을 뭉쳐진 별밭과 **함께** 옮기고, 씬 버퍼·오브젝트 큐브맵 재렌더는 우리 워프 창 안에서 돌아 위치가 일관됩니다. 렌징 결과는 근거리 캠 깊이 기준 "하늘"이라 도플러 틴트/빔도 그대로 받습니다. 실제 지오메트리 방식이라 TAA·플레어·마스크와 상호작용할 표면도 없습니다. **알려진 한계** - 자체 갤럭시 큐브맵을 시작 시 정지 좌표계로 1회 캡처하므로, 고β에서 렌즈 링 내부 별 패턴만 수차 전 하늘로 남습니다(링 안팎 별 밀도 불일치, 우리 쪽에서 수정 불가). 확인할 엣지 케이스 - 아광속 상태 퀵로드 시 우리 워프가 그쪽 시작 캡처에 섞일 수 있습니다. |
| **Scatterer** *(검증됨)* | 대기 산란 + 자체 카메라 레벨 TAA | `ADAPTER` - 플레어/대기는 코드 없이도 우리 천체 수차 렌더 창을 따라옵니다(카메라별 읽기, 소스 검증). **내장 TAA는** 1.1.0부터 기본 하늘 그레이딩 경로와 **공존합니다**. 그레이딩이 함선/TAA 단계보다 먼저 돌기 때문에, Scatterer TAA는 그 그레이딩된 하늘을 매끄럽게 다듬을 뿐 서로 부딪히지 않습니다(예전 `ScattererTAAAdapter` 서스펜드는 제거됨). **대체 선플레어**(근거리 카메라가 그리는 소프트 가산 스크린 메시)는 매 프레임 원본에서 억제하고 단독 캡처한 뒤(`ScattererFlareCapture`, `SunFlare.sunflareGameObject` @ c2d0b0f 기준 소프트 타입, 가림/페이드는 정점 단계에서 게이트되므로 보존됨), 전용 셰이더 패스가 도플러 틴트를 입혀 다시 얹습니다 - 후방은 붉고 어둡게, 전방은 원래 밝기 그대로, 선체 겹침도 포함합니다. 리플렉션이 어긋나면 한 프레임 안에 스톡 플레어로 내려앉습니다. |
...
| **MechJeb** *(소스 확인 @ c25bbb3)* | 오토파일럿 / 기동 실행 · GPL-3.0 | `INTEGRATE` (v1.2) - 두 추력 서브시스템이 모두 엔진의 광고값을 읽기 때문에 번 시간 추정, 점화 시점, 스로틀 명령, 착륙/상승 계산이 우리의 F/γ³ 감쇠를 보지 못했습니다: `MechJebAdapter`가 실시간 `VesselState` getter와 예측용 연료 흐름 시뮬레이션이 유효 추력을 보고하게 만듭니다. 번 컷오프는 원래부터 자가 보정됩니다(스톡: 라이브 노드 벡터, Principia: 측정 가속도) - 패치 전의 증상은 잘못된 ΔV가 아니라 엉터리 예상 시간과 늦은 점화였습니다. 자세: 노드 점화가 피드백 게이팅되므로(*측정된* 정렬을 기다림) 선회가 느려지면 ALIGNING이 길어질 뿐이고, 핵심 발견대로 토크 기반 감소를 선호합니다. 알려진 한계: Flight Recorder 그래프 창은 상대론적 수치에서 축 라벨 배열을 넘겨 IndexOutOfRange를 쏟아냅니다(MechJeb 자체 UpdateScale, @ 5eef373f 확인) - 고속에서는 그 창을 닫으세요. 개발 도구까지 패치하는 건 과해서 어댑터로 고치지 않습니다. |
| **kOS** *(소스 확인 @ 8f281a4)* | 스크립트 조종 · GPL-3.0 | `INTEGRATE` (v1.2) - 모든 추력 접미사(SHIP/ENGINE MAXTHRUST 계열, SHIP:THRUST)가 광고값을 읽어서 표준 `dv/(F/m)` 번 타임 스크립트가 γ³배 과소평가했습니다: `KOSAdapter`가 초크 포인트 두 곳을 스케일해 해당 접미사들이 유효 추력을 보고합니다(비추력 접미사는 분자·분모가 같이 스케일돼 원값 유지 - 구조적으로 상쇄). 알려진 한계: `ENGINE:THRUST`는 인라인 람다라 패치 불가, 원값 그대로입니다. 측정 기반 경로(`SHIP:SENSORS:ACC`, 속도 차분)는 원래부터 정확했습니다 - 상대론에서 안전한 텔레메트리 선택지입니다. cooked 스티어링은 측정된 `angularVelocity`에 대한 폐루프입니다(우아함). 스크립트가 `ANGLEERROR`를 폴링하는 대신 `WAIT n.` 선회 *마감 시각*을 하드코딩할 때만 깨집니다 → 사용자용 README 노트. |
| **스톡 네브볼 번 타이머** | KSP 1.12.x 내장(비공개 소스) | `INTEGRATE` (v1.2) - "Est. Burn"과 번 시작 카운트다운은 VesselDeltaV 시뮬레이션(광고 추력)에서 나옵니다: `NavballBurnTimeAdapter`가 추정치를 ×γ³로 늘립니다. 현재 속도 스냅샷 근사이고(긴 번에서는 γ가 변함) 이 계열에서 유일하게 확신도가 중간인 패치라(비공개 스톡 메서드, 본문 미확인) 전용 설정 게이트 `compatStockBurnTimer`가 있습니다. 스톡 ΔV 앱 자체는 의도적으로 패치하지 않습니다(서드파티 소비자가 너무 많음). |
...
| **kOS** `KSP-KOS` @ `8f281a4` (GPL-3.0) | postfix `ModuleEnginesExtensions.GetThrust`(`ref float __result`) + `VesselUtils.GetCurrentThrust`(`ref double __result`) | 함선별 `__result ×= 1/γ³`. static 초크 하나가 엔진·함선 스코프의 MAX/AVAILABLE/POSSIBLE-THRUST 계열 전체를, 두 번째가 SHIP:THRUST를 커버합니다. `KOSAdapter`로 탑재(v1.2). |
| **MechJeb2** `MuMech` @ `c25bbb3` (GPL-3.0) | postfix `VesselState` getter `ThrustAvailable`/`ThrustMinimum`/`ThrustCurrent`; postfix `FuelFlowSimulation.Run`에서 `Segments[i].Thrust/MaxThrust/MinThrust` 스케일 | ×1/γ³. 시뮬레이션 내부는 `[AggressiveInlining]` 투성이라 인라인되지 않는 `Run` 오버라이드만이 안전한 타깃입니다. 백그라운드 스레드에서 돌기 때문에 배율은 매 FixedUpdate마다 메인 스레드에서 캡처합니다(volatile). `MechJebAdapter`로 탑재(v1.2). |
| **스톡 네브볼** KSP 1.12.x (비공개 소스) | postfix `NavBallBurnVector.CalculateBurnTime()`(private, `ref double __result`) | 유한 양수일 때만 `__result ×= γ³`. 확신도 중간(본문 미확인), 생태계에 알려진 패치 충돌 없음, `compatStockBurnTimer`로 게이팅. `NavballBurnTimeAdapter`로 탑재(v1.2). |

**안정된 외부 표면.** `Relativity.RelativityApi`(static, `ApiVersion` = 1): `GetGamma(Vessel)`과
`GetThrustMultiplier(Vessel)`(1/γ³). 레이어가 비활성이면 둘 다 항등값입니다. Principia 포크의 워프 번
수확이 리플렉션으로 조회하는 표면(ROADMAP)이자 우리 호환 어댑터들이 쓰는 표면입니다. 메인 스레드
전용이고, 시그니처는 동결이며, 호환성이 깨지는 변경은 `ApiVersion`을 올립니다.
