...
| **Scatterer** *(grounded)* | Atmospheric scattering + its own camera-level TAA | `ADAPTER` - its flare/atmosphere follow our body-aberration render window with no code (per-camera reads, source-verified). Its **built-in TAA** (default ON in its settings, a plain MonoBehaviour on the near/far/scaled cameras - not a PostProcessLayer) jitters the projection each frame, which the visual's beam amplification exposes as sky shimmer: `ScattererTAAAdapter` suspends exactly those behaviours while the visual is active and restores them (+history reset) after, soft-typed against `Scatterer.TemporalAntiAliasing` @ c2d0b0f, idle without Scatterer. `dopplerSuppressScattererTAA=false` opts out. Its SMAA is unaffected. Its **replacement sunflare** (a soft-additive screen mesh the near camera draws) is additionally captured alone each frame (`ScattererFlareCapture`, soft-typed against `SunFlare.sunflareGameObject` @ c2d0b0f) so the visual can un-blend it, process the clean sky, and re-blend it Doppler-shifted - exact per-pixel flare treatment; any reflection surprise degrades to the cone-window fallback. |
