...
| `attitudeExponent` | `2` | γ-exponent on reaction-wheel/RCS **torque** ([[The Physics#4-attitude-turning-slows-as-1γ]]). `2` = torque ×1/γ², which makes a torque-limited slew take ~γ× longer (the physical rotation-rate ×1/γ); `1` gives only ~√γ×; `0` disables the attitude slowdown. |
